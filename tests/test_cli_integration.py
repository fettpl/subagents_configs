...
class CliIntegrationTests(unittest.TestCase):
        ...
        with private_tempdir() as directory:
            ...
            for payload in payloads:
                for failure in (ValueError(payload), OSError(payload)):
                    out, err = io.StringIO(), io.StringIO()
                    with patch(
                        "subagents_configs.orchestrator._plan", side_effect=failure
                    ):
                        status = run(
                            "install",
                            request_args,
    ...
    def test_render_and_output_failures_are_stable_and_sanitized(self):
        with private_tempdir() as directory:
            ...
            with (
                patch(
                    "subagents_configs.orchestrator._plan", return_value=BrokenPlan()
                ),
                patch(
                    "subagents_configs.orchestrator.render_plan", return_value="plan\n"
                ),
            ):
                status = run(
                    "install",
